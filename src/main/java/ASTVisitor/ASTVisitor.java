...
public abstract class ASTVisitor {
    ...
    void visit(BreakStatementNode node) throws SemanticError {}

    void visit(ClassDefinitionNode node) throws Exception {
        for (ExpressionDefinitionNode item : node.memberVariableList) visit(item);
    ...
    void visit(ContinueStatementNode node) throws SemanticError {}
    ...
    void visit(ExpressionStatementNode node) throws Exception {
        ...
        else if (node instanceof ExpressionDefinitionNode) visit((ExpressionDefinitionNode)node);
...
//        if (node.exprType != null) visit((TypeNode) node.exprType); ATTENTION
    ...
    void visit(MethodDefinitionNode node) throws Exception {
        ...
        for (ExpressionDefinitionNode item : node.formalArgumentList) visit(item);
        visit(node.block);
    }
    ...
    void visit(ProgramNode node) throws Exception {
        for (ASTNode item : node.childrenList) {
            ...
            else visit((ExpressionDefinitionNode)item);
        }
    }
    ...
    void visit(NonArrayTypeNode node) throws SemanticError {
